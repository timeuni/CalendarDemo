...
class CalendarLogic: NSObject {
    var today = NSDate() //今天的日期
    var before = NSDate() //之后的日期
    var select = NSDate() //选择的日期
    ...
    func reloadCalendarView(date: NSDate, selectDate: NSDate, days_number: NSInteger) -> [AnyObject]{
        ...
        for i in 0..<months+1 {
            ...
            calendarMonth.insert(calendarDays, atIndex: i)
        }
    ...
    func calculateDaysInPreviousMonthWithDate(date:NSDate, var array:[AnyObject]) -> [AnyObject]{
        let weeklyOrdinality = date.firstDayOfCurrentMonth().weeklyOrdinality()//计算这个的第一天是礼拜几,并转为int型
        let dayInThePreviousMonth = date.dayInThePreviousMonth() //上一个月的NSDate对象
    ...
    //计算下月份的天数
    func calculateDaysInFollowingMonthWithDate(date: NSDate, var array: [AnyObject]) -> [AnyObject]{
    ...
    //计算当月的天数
    func calculateDaysInCurrentMonthWithDate(date: NSDate, var array:[AnyObject]) -> [AnyObject]{
    ...
    func changStyle(calendarDay: CalendarDayModel){
    ...
    func LunarForSolarYear(calendarDay: CalendarDayModel){
        let solarYear = LunarForSolarYear(calendarDay.year, Month: calendarDay.month, Day: calendarDay.day)
        let solarYear_arr = solarYear.componentsSeparatedByString("-")
    ...
    func LunarForSolarYear(var wCurYear: NSInteger, var Month:Int, var Day:Int) -> String{
        ...
        nTheDate = (wCurYear - 1921) * 365 + (wCurYear - 1921) / 4 + Day + WMonthAdd[Month - 1] - 38
        if((wCurYear % 4 == 0) && (Month > 2)){
            nTheDate = nTheDate + 1
        }
        ...
        if (k == 12)
        {
            if (Month == wNongliData[m] / 65536 + 1){
                Month = 1 - Month}
            else if (Month > wNongliData[m] / 65536 + 1){
                Month = Month - 1}
        }
        ...
        var szNongliMonth = ""
        if (Month < 1){
            szNongliMonth = "闰\(cMonName[-1 * Month])"
        }else{
            szNongliMonth = "\(cMonName[Month])"
        }
        
        //生成农历日
        let szNongliDay = cDayName[Day]
        return "\(szNongliMonth)-\(szNongliDay)"
    }
    
    func selectLogic(day: CalendarDayModel){
